...
// getAgencyResourcesAndCalamities
export const getAgencyResourcesAndCalamities = async (req, res) => {
  try {
    const agencyId = req.user._id; // Change the parameter name to agencyId
    console.log(agencyId);
    ...
    // Retrieve all calamities in which the agency has worked
    const calamities = await Disaster.find({ agencies: agency._id });

    res.status(200).json({
      message: "Agency resources and calamities retrieved successfully",
      agency,
      resources,
      calamities,
    });
    ...
    res.status(500).json({
      success:false,
      message: "Error retrieving agency resources and calamities",
      error,
    });
  }
};
...
export const findAgency = async(req, res) => {
  try{
    ...
    res.status(200).json({ agency, resources, calamities });
  }catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Internal server error' });
  }
