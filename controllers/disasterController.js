...
// Fetch multiple calamities
export const fetchCalamities = async (req, res) => {
  try {
    const filter = filterResults(req.query);
    const calamities = await Disaster.find(filter).sort({ timestamp: -1 });

    res.status(200).json({ success: true,message:"All the disaster fetched successfully", calamities });
  } catch (error) {
    res.status(500).json({ success:false,message: "Internal server Error",error });
  }
};
